...
export interface StrategyConfig {
    enabled: boolean;
    symbol: string;
    riskPerTrade: number; // Risk amount per trade (e.g., $100)
    maxPositionSize: number; // Max position size as % of account
    vwapPeriod: number; // VWAP calculation period
    stopLossMultiplier: number; // Stop loss as multiple of risk (e.g., 1R)
    takeProfitMultiplier: number; // Take profit as multiple of risk (e.g., 2R)
}
...
export class VWAPStrategy {
    ...
    private calculateVWAP(bars: BarData[], period: number): number {
    ...
    private checkEntrySignal(currentPrice: number, previousVWAP: number, currentVWAP: number): boolean {
    ...
    private checkExitSignal(currentPrice: number, currentVWAP: number): boolean {
    ...
    public processBar(bar: BarData): { signal: string; trade?: Trade } {
    ...
    public getStatus(): { active: boolean; position: Trade | null; vwap: number } {
        return {
...
export class StrategyEngine {
    private ibService: typeof ibService;
    private strategies: Map<string, VWAPStrategy> = new Map();
    private isRunning: boolean = false;

    constructor(ibServiceInstance: typeof ibService) {
        this.ibService = ibServiceInstance;
    }

    // Add a new strategy
    public addStrategy(symbol: string, config: StrategyConfig): void {
    ...
    // Remove a strategy
    public removeStrategy(symbol: string): void {
    ...
    public processMarketData(symbol: string, bar: BarData): void {
        const strategy = this.strategies.get(symbol);
        if (strategy) {
            ...
            if (result.signal === 'ENTRY' && result.trade) {
                ...
                this.executeOrder({ ...result.trade, side: 'SELL' });
            }
        }
    }

    // Execute order through IB API
    private async executeOrder(trade: Trade): Promise<void> {
        try {
    ...
    public getStrategiesStatus(): { [symbol: string]: any } {
        const status: { [symbol: string]: any } = {};
        for (const [symbol, strategy] of this.strategies.entries()) {
            status[symbol] = {
                ...strategy.getStatus(),
                metrics: strategy.getMetrics(),
                trades: strategy.getTrades()
            };
        }
        return status;
    }
